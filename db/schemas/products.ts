...
export const categories = sqliteTable("product_categories", {
  ...
  parentId: integer("parent_id").references(
    (): AnySQLiteColumn => categories.categoryId
  ),
});
...
export const idxProductVariantsSku = index("idx_products_sku").on(
  productVariants.sku
);
...
export const productAttributes = sqliteTable(
  "product_attributes",
  {
    attributeId: integer("attribute_id").primaryKey({ autoIncrement: true }),
    productId: integer("product_id")
      .notNull()
      .references(() => products.productId, { onDelete: "cascade" }),
